...
module Pitchfork
  class TestSharedMemory < Test
    ...
    def test_for_fd_exec
      ...
      file.write(<<~'RUBY')
        require "pitchfork"
        page = Pitchfork::MemoryPage.for_fd(Integer(ENV["PITCHFORK_MEMORY_FD"]))
        if page[0] == 1234
          page[0] = 4321
        else
          exit 1
      ...
      in_child do
        @page[0] = 1234
        ENV["PITCHFORK_MEMORY_FD"] = @page.fileno.to_s
        Process.exec(RbConfig.ruby, file.path)
      end
    ...
    def test_close_on_exec
      ...
      file.write(<<~'RUBY')
        require "pitchfork"
        begin
          Pitchfork::MemoryPage.for_fd(Integer(ENV["PITCHFORK_MEMORY_FD"]))
        rescue Errno::EBADF
          exit 0
        else
          exit 1
